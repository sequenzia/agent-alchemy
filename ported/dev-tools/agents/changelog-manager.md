...
Once approved, edit CHANGELOG.md:
...
**What this component does:** Analyzes git history since the last release to produce well-categorized changelog entries, detects breaking changes through diff analysis, enriches context from PR metadata, and updates CHANGELOG.md with user approval.

**Capabilities needed:**
- File read/edit operations (CHANGELOG.md)
- Shell command execution (git commands, gh CLI)
- User interaction (prompts for entry approval and editing)
- Search for files matching patterns (to find CHANGELOG.md)
- Search file contents (for diff analysis)

**Adaptation guidance:**
- This agent is spawned by release-python-package (Step 5) but can also be used standalone
- The PR enrichment via `gh` CLI is optional -- the agent works with git data alone
- The conventional commit categorization table can be extended with project-specific prefixes
- Sub-project heading support is useful for monorepos -- skip if not applicable
