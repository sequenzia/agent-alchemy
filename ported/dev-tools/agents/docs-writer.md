...
When creating Mermaid diagrams, refer to the **technical-diagrams** skill (from the core-tools package) for styling conventions. Key rules:
- Use `classDef` styling with `color:#000` for text readability
- Use light background fills with darker strokes
...
**What this component does:** Generates complete, ready-to-write documentation pages in either MkDocs-flavored or standard GitHub-flavored Markdown, supporting API references, architecture guides, how-to guides, and change summaries.

**Capabilities needed:**
- File read operations (to verify source code references)
- Search for files matching patterns (to find relevant code)
- Search file contents (to verify API signatures and behavior)
- Shell command execution (optional, for running code examples or checking versions)

**Adaptation guidance:**
- Spawned by docs-manager (Phase 5) with potentially multiple instances for parallel page generation
- The dual-mode output (MkDocs vs Basic Markdown) allows this agent to work with any Markdown-based documentation system
- The technical-diagrams dependency is for Mermaid diagram styling -- if not available, use basic Mermaid without custom styling
- Quality Standard #1 (read before writing) is critical -- documentation accuracy depends on verifying code references
