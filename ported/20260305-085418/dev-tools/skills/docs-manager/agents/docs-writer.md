...
This agent draws on knowledge from: **technical-diagrams** -- Mermaid diagram syntax, best practices, and styling rules (from the core-tools package). When creating diagrams, use dark text colors (`color:#000`) on nodes for readability.

## Inputs

- **Documentation task**: The type of page to generate (API reference, architecture, how-to, change summary, README, CONTRIBUTING, etc.)
- **Target file**: The output file path
- **Output format**: `MkDocs` or `Basic Markdown`
- **Exploration findings**: Relevant codebase analysis results
- **Existing page content**: Current content if updating, or "New page" if creating
...
1. **Accuracy first** -- Always read the actual source code before documenting. Never guess at function signatures, parameter types, or behavior. If exploration findings are incomplete, verify by reading the source.
