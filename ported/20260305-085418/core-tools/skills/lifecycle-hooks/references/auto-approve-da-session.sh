...
#
# This script expects action context as JSON on stdin (with tool_name and tool_input fields).
# It outputs a JSON permission decision for matching operations, or nothing (no opinion) for others.
#
...
#
# IMPORTANT: This hook must NEVER exit non-zero. A non-zero exit causes the agent platform
