...
#
# Purpose: Prevents permission prompts from blocking autonomous deep-analysis
# session management. Only approves operations on session-specific directories;
# all other operations pass through to the normal permission flow.
#
# Expected input: JSON on stdin with the following structure:
#   { "tool_name": "Write|Edit|Bash", "tool_input": { "file_path": "..." } }
#   or for Bash:
...
#
# This hook must NEVER exit non-zero. A non-zero exit causes the platform
