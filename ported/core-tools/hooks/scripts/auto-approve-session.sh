...
#
# Purpose:
# Prevents permission prompts from interrupting the autonomous deep-analysis
# workflow. Approves operations targeting session working directories and
# exploration cache without requiring user confirmation.
#
# Expected input:
# Receives action context as JSON on stdin with fields:
#   - tool_name: The type of action being performed (e.g., "Write", "Edit", "Bash")
#   - tool_input: Action-specific parameters (e.g., file_path for Write/Edit, command for Bash)
#
...
#
# IMPORTANT: This hook must NEVER exit non-zero. A non-zero exit would cause
# the harness to fall through to the normal permission prompt, breaking
