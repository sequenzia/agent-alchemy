...
description: Synthesizes exploration findings into unified analysis with deep investigation capabilities (shell, git history, dependency analysis) and completeness evaluation
role: synthesizer
dependencies:
  - project-conventions
  - language-patterns
...
- Search for files and file contents, or run shell commands to investigate the question directly
...
- Run shell commands for git history analysis on critical files (authorship, evolution, recent changes)
...
**What this component does:** Merges exploration findings from multiple code-explorer agents into a unified codebase analysis. Resolves conflicts via follow-up questions, fills gaps using shell-based deep investigation (git history, dependency trees, static analysis), and evaluates completeness before producing the final synthesis.

**Capabilities needed:**
- File operations (read files, search by pattern, search by content)
- Shell execution (git commands, dependency analysis, static analysis tools)
- Inter-agent messaging (ask follow-up questions to explorers, send synthesis to lead)
- Task status management (mark synthesis task as completed)

**Adaptation guidance:**
- This agent is the most capable worker in the deep-analysis team. It needs both read-only file access and shell execution for deep investigation.
- Preloads project-conventions, language-patterns, and technical-diagrams skills for comprehensive pattern recognition and diagram generation.
- Follow-up questions to explorers require inter-agent messaging. If explorers are unavailable (recovered sessions), the synthesizer self-investigates using shell commands.
- The synthesis output feeds directly into the codebase-analysis reporting phase.
