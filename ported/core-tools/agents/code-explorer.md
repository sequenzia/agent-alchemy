...
You are part of a team with other explorers and a synthesizer. Respond to questions from team members as needed.
...
**What this component does:** Independently explores a designated focus area of a codebase, finding relevant files, tracing execution paths, mapping architecture, and reporting structured findings. Works as part of the deep-analysis hub-and-spoke team.

**Capabilities needed:**
- File operations (read files, search by pattern, search by content)
- Shell execution (for tracing dependencies, running analysis commands)
- Inter-agent messaging (receive assignments, respond to follow-ups from synthesizer)
- Task status management (mark tasks as in-progress/completed)

**Adaptation guidance:**
- This agent is spawned by the deep-analysis skill as a worker in a hub-and-spoke topology. It receives a focus area assignment and works independently.
- The agent needs read-only file access for exploration. It does not write or modify code.
- Team communication (acknowledgments, follow-up responses) should be mapped to your platform's inter-agent messaging mechanism.
- Preloads the project-conventions and language-patterns skills for pattern recognition during exploration.
