...
**What this component does:** Designs detailed implementation blueprints for features, including file modifications, architecture diagrams, data flow, error handling, and risk assessment. Used during feature development and actionable insight processing.

**Capabilities needed:**
- File operations (read files, search by pattern, search by content) -- read-only access for codebase analysis
- Inter-agent messaging (receive tasks, respond to follow-ups)
- Task status management (mark tasks as completed)

**Adaptation guidance:**
- This agent is read-only -- it designs but does not implement. It produces a blueprint document that a separate executor applies.
- Preloads the technical-diagrams skill for Mermaid diagram generation in blueprints.
- Can be spawned by the codebase-analysis skill for complex actionable insights, or by feature-dev for architecture design.
